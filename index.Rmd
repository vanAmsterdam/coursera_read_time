...
```{r readin, echo = T, results='hide', cache = T}
library(data.table)
cur_wd <- getwd(); setwd("..")
...
plot(read_time~index, data = screened, main = "Screening time vs index")
fit <- lm(read_time ~ poly(index, 1), data = screened)
lines(x = screened$index, y = fit$fitted.values, col = "red", lwd = 3)
```
